...
for t in range(200):
    ...
    for ii in range(TIMESTEPS):
        us_1[ii, :] = -0.9*np.transpose(alphas_1[ii]) - Ps_1[ii][1][0:4] @ (robot1.state.detach().numpy() - x_ref_1)
        us_2[ii, :] = -0.9*np.transpose(alphas_2[ii]) - Ps_2[ii][1][4:8] @ (robot2.state.detach().numpy() - x_ref_2)
